...
class ThinkingModule:
    ...
    def analyze_situation(self, state: GameState, vision: List[List[str]]) -> Dict[str, Any]:
    ...
    def create_plan(self, goal: str, state: GameState, analysis: Dict[str, Any]) -> List[ActionType]:
        ...
        if goal == "survive":
            ...
            if random.random() < 0.3:
                plan.append(ActionType.TURN_LEFT if random.random() < 0.5 else ActionType.TURN_RIGHT)
    ...
    def update_meta_knowledge(self, state: GameState, action: ActionType, result: str):
        ...
        if pos_key not in self.meta_knowledge['map_knowledge']:
            self.meta_knowledge['map_knowledge'][pos_key] = {'visited_count': 0, 'resources_found': []}
