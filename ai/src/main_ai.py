...
from Classes import ActionType, Experience, GameState, Position, ResourceType
...
class ZappyAI:
    ...
    def create_state_vector(self) -> np.array:
        ...
        while (
            len(vision_flat) < 35
        ):  # 9 tiles * 7 resources - 28, but we want 35 for round number
    ...
    def calculate_reward(
        self, action: ActionType, result: str, previous_state: GameState
    ) -> float:
        ...
        # Food management
        food_diff = self.state.inventory.get("food", 0) - previous_state.inventory.get(
            "food", 0
        )
        if food_diff > 0:
        ...
        for resource in [
            ...
            diff = self.state.inventory.get(resource, 0) - previous_state.inventory.get(
                resource, 0
            )
            if diff > 0:
    ...
    def think_and_decide(self) -> ActionType:
        ...
        situation_analysis = self.thinking_module.analyze_situation(
            self.state, self.last_look_result
        )

        # Check if we have a successful strategy for this situation
        situation_key = (
            f"level_{self.state.level}_food_{self.state.inventory.get('food', 0) // 10}"
        )
    ...
    def learn_from_experience(self, experience: Experience):
        ...
        try:
            ...
            if not experience.done:
                # Q-learning update
                next_q_values = self.neural_network.predict(
                    experience.next_state.reshape(1, -1)
                )
    ...
    def process_response(
        self, response: str, previous_state: GameState, action: ActionType
    ) -> bool:
    ...
    def print_thinking_process(
        self, action: ActionType, situation_analysis: Dict[str, Any]
    ):
    ...
    def adaptive_learning_adjustment(self):
        ...
        recent_rewards = (
            list(self.reward_history)[-100:]
            if len(self.reward_history) >= 100
            else list(self.reward_history)
        )
        ...
        if recent_rewards:
            ...
            if avg_reward < -10:  # Very poor performance
                self.neural_network.learning_rate = min(
                    0.01, self.neural_network.learning_rate * 1.1
                )
            elif avg_reward > 10:  # Very good performance
                self.neural_network.learning_rate = max(
                    0.0001, self.neural_network.learning_rate * 0.95
                )
    ...
    def should_fork(self) -> bool:
        ...
        if self.state.inventory.get("food", 0) > 100:
            ...
            current_stones = sum(
                self.state.inventory.get(stone, 0) for stone in required_stones.keys()
            )

            if (
                current_stones > sum(required_stones.values()) * 2
            ):  # Have 2x what we need
                return True
    ...
    def attempt_fork(self):
        ...
        if self.should_fork():
            ...
            print(
                f"🍴 Attempting to fork! (Food: {self.state.inventory.get('food', 0)}, Level: {self.state.level})"
            )
            self.send_command("Fork")
            return True
        return False
        ...
        try:
            while True:
                ...
                if turn_counter - last_save >= 500:
                    ...
                    print(
                        f"   Total Rewards: {self.performance_metrics['total_rewards']:.2f}"
                    )
                    print(
                        f"   Success Rate: {self.performance_metrics['successful_actions'] / max(1, self.performance_metrics['successful_actions'] + self.performance_metrics['failed_actions']) * 100:.1f}%"
                    )
                    print(
                        f"   Resources Collected: {self.performance_metrics['resources_collected']}"
                    )
                    print(
                        f"   Elevations: {self.performance_metrics['elevations_completed']}"
                    )
                    print(
                        f"   Forks: {self.performance_metrics['forks_successful']}/{self.performance_metrics['forks_attempted']}"
                    )
                    print(f"   Food: {self.state.inventory.get('food', 0)}")
