...
class ZappyAI:
    ...
    def connect(self):
        ...
        try:
            ...
            print(f"Connected to server. Team slots: {self.state.team_slots}, World: {self.state.world_width}x{self.state.world_height}")
            return True
    ...
    def find_resources_in_vision(self, tiles: List[List[str]]) -> List[Tuple[int, str]]:
        ...
        for tile_idx, tile_content in enumerate(tiles):
            for item in tile_content:
                if item in ["food", "linemate", "deraumere", "sibur", "mendiane", "phiras", "thystame"]:
            ...
            if self.current_goal in ["find_food", "gather"]:
                for tile_idx, resource in resources_found:
                    if (self.current_goal == "find_food" and resource == "food") or \
                       (self.current_goal == "gather" and resource in self.target_resources):
                        if tile_idx == 0:
