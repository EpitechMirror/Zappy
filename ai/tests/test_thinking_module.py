...
from Classes import ActionType, GameState, Position, ResourceType
...
class TestThinkingModule(unittest.TestCase):
    ...
    def test_analyze_situation_normal(self):
        ...
        analysis = self.thinking_module.analyze_situation(
            self.sample_state, self.sample_vision
        )
    ...
    def test_analyze_situation_low_food(self):
        ...
        low_food_state = GameState(level=1, inventory={"food": 15})  # Low food

        analysis = self.thinking_module.analyze_situation(
            low_food_state, self.sample_vision
        )
    ...
    def test_analyze_situation_very_low_food(self):
        ...
        very_low_food_state = GameState(level=1, inventory={"food": 5})  # Very low food

        analysis = self.thinking_module.analyze_situation(
            very_low_food_state, self.sample_vision
        )
    ...
    def test_analyze_situation_elevation_opportunity(self):
        ...
        analysis = self.thinking_module.analyze_situation(
            ready_state, self.sample_vision
        )
    ...
    def test_calculate_elevation_needs_level_2(self):
        ...
        level_2_state = GameState(
            level=2, inventory={"food": 10, "linemate": 0, "deraumere": 1, "sibur": 0}
        )
    ...
    def test_create_plan_survive(self):
        ...
        plan = self.thinking_module.create_plan(
            "survive", self.sample_state, high_urgency_analysis
        )
    ...
    def test_create_plan_gather_food_current_tile(self):
        ...
        plan = self.thinking_module.create_plan(
            "gather_food", self.sample_state, food_analysis
        )
    ...
    def test_create_plan_gather_resources(self):
        ...
        resource_analysis = {
            "urgency_level": 2,
            "resource_availability": {"deraumere": [3], "sibur": [3], "phiras": [6]},
        }

        # Mock the calculate_elevation_needs to return specific needs
        with patch.object(
            self.thinking_module, "calculate_elevation_needs"
        ) as mock_needs:
            mock_needs.return_value = {"deraumere": 1, "sibur": 1}

            plan = self.thinking_module.create_plan(
                "gather_resources", self.sample_state, resource_analysis
            )
    ...
    def test_create_plan_elevate(self):
        ...
        elevation_analysis = {"urgency_level": 3, "opportunities": ["can_elevate"]}

        plan = self.thinking_module.create_plan(
            "elevate", self.sample_state, elevation_analysis
        )
    ...
    def test_create_plan_explore(self):
        ...
        exploration_analysis = {"urgency_level": 1, "opportunities": []}

        with patch("random.random", return_value=0.1):  # Force turn
            plan = self.thinking_module.create_plan(
                "explore", self.sample_state, exploration_analysis
            )
    ...
    def test_update_meta_knowledge_action_patterns(self):
        ...
        self.thinking_module.update_meta_knowledge(
            state_with_turn, ActionType.MOVE_FORWARD, "ok"
        )

        # Check that action pattern is recorded
        self.assertIn(
            "MOVE_FORWARD", self.thinking_module.meta_knowledge["time_patterns"]
        )
        self.assertIn(
            initial_turn,
            self.thinking_module.meta_knowledge["time_patterns"]["MOVE_FORWARD"],
        )
    ...
    def test_update_meta_knowledge_map_knowledge(self):
        ...
        self.thinking_module.update_meta_knowledge(
            state_with_pos, ActionType.LOOK, "ok"
        )
        ...
        self.assertEqual(
            self.thinking_module.meta_knowledge["map_knowledge"][pos_key][
                "visited_count"
            ],
            1,
        )
    ...
    def test_update_meta_knowledge_multiple_visits(self):
        ...
        for _ in range(3):
            self.thinking_module.update_meta_knowledge(
                state, ActionType.MOVE_FORWARD, "ok"
            )
