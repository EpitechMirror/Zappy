...
class TestZappyAIMain(unittest.TestCase):
    ...
    @patch("zappy_ai.ZappyAI")
    @patch(
        "sys.argv", ["zappy_ai.py", "-p", "8080", "-n", "my_team", "-H", "remote_host"]
    )
    ...
    @patch("zappy_ai.ZappyAI")
    @patch(
        "sys.argv", ["zappy_ai.py", "-p", "4242", "-n", "test_team", "--no-learning"]
    )
    ...
    def test_argument_parser_configuration(self):
        ...
        parser = argparse.ArgumentParser(
            description="Zappy AI Client with Neural Networks"
        )
        ...
        parser.add_argument(
            "--no-learning", action="store_true", help="Disable learning"
        )
        ...
        args = parser.parse_args(
            ["-p", "8080", "-n", "my_team", "-H", "example.com", "--no-learning"]
        )
    ...
    def test_main_no_learning_message(self, mock_print, mock_zappy_ai_class):
        ...
        with patch(
            "sys.argv",
            ["zappy_ai.py", "-p", "4242", "-n", "test_team", "--no-learning"],
        ):
            zappy_ai.main()
    ...
    def test_script_name_in_argv(self):
        ...
        try:
            ...
            with patch(
                "sys.argv", ["enhanced_zappy_ai.py", "-p", "4242", "-n", "test_team"]
            ):
