...
class TestNeuralNetwork(unittest.TestCase):
    ...
    def test_network_initialization(self):
        ...
        self.assertEqual(self.nn.layers[0]['weight'].shape, (4, 8))  # input to first hidden
        self.assertEqual(self.nn.layers[1]['weight'].shape, (8, 4))  # first to second hidden
        self.assertEqual(self.nn.layers[2]['weight'].shape, (4, 2))  # second hidden to output
    ...
    def test_backward_pass_shapes(self):
        ...
        original_shapes = [(layer['weight'].shape, layer['bias'].shape) for layer in self.nn.layers]
