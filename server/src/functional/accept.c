...
static void handle_client_disconnection(struct pollfd *fds, int *client_count,
    int i)
{
    close(fds[i].fd);
...
{
    bool disconnected = handle_client_data(&ctx->clients,
...
static poll_context_t init_ctx(int server_fd, server_config_t *conf)
{
    poll_context_t ctx = {
        .server_fd = server_fd,
        .conf = conf,
...
{
    int poll_count;
    poll_context_t ctx = init_ctx(server_fd, conf);

    set_non_blocking(server_fd);
    ctx.fds[0].fd = server_fd;
    ctx.fds[0].events = POLLIN;
    while (1) {
        poll_count = poll(ctx.fds, ctx.client_count, -1);
        if (poll_count == -1) {
            perror("poll");
            break;
        }
